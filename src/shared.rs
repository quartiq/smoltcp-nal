    ...
    S: embedded_nal::TcpClientStack,
{
    ...
    forward! {is_open(socket: &S::TcpSocket) -> Result<bool, S::Error>}
    forward! {may_send(socket: &S::TcpSocket) -> Result<bool, S::Error>}
    forward! {may_receive(socket: &S::TcpSocket) -> Result<bool, S::Error>}
